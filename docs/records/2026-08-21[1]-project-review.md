...
> Состояние на 2026-08-22: ревью завершено на срезе `a175dd7`. Находки №1,
> №2, №3, №5, №6, №7, №8, №9 и №12 исправлены; №4 принят как исходный
> контракт и документирован; №10 принята владельцем как известное
> ограничение 0.7.0 и задокументирована, safe mode отложен в бэклог.
> Остаются 5 исходных находок — все Low.
...
**Вердикт (2026-08-22): поведение принято владельцем, граница доверия
задокументирована; safe mode отложен в бэклог.** Воспроизведение подтверждено
на живом коде: `ESC` из сообщения и из значений данных доходит до вывода
нетронутым, прочие C0 экранируются в видимые символы. OSC-последовательность,
закрытая ST (`ESC \`), доходит целиком — включая OSC 8, то есть подделанную
кликабельную ссылку; закрытая BEL остаётся незакрытой, потому что BEL
экранируется. В JSONL `ESC` доезжает до строки, но `jsonEncode` пишет его как
`\u001b`, поэтому сам файл инертен, а последовательность оживает только при
повторной печати расшифрованного сообщения в терминал.

Заявленный в находке opt-in оказался частичным, и это ключевая причина
отложить. `ControlCodeFormatter(excludeEscCode: false)` действительно
экранирует `ESC` в сообщении и в значениях обычных контейнеров, но не годится
для свойств `Loggable`/`LoggableData`: `Prop.toLogString` зовёт value-форматтер
второй раз поверх уже стилизованной строки, поэтому вместе с инъекцией
экранируются собственные коды темы и вывод свойства рассыпается. Это тот самый
блокер «двойное форматирование в `Prop.toLogString`», записанный владельцем в
`docs/backlog.md`; там же safe mode увязан с работой по default/force
конфигам. Полноценный безопасный режим требует переноса экранирования на лист
и потому остаётся отдельной работой, а не настройкой этой версии.

Поведение не менялось. Граница доверия названа явно в README (новый раздел
«11. Untrusted Text and Terminal Output»), в dartdoc `ControlCodeFormatter`,
`BbCodeFormatter`, `LogMainTheme.valueFormatter`/`messageFormatter`, в
`docs/architecture.md` и в `CHANGELOG.md`. Одиннадцать характеризующих тестов
в `test/preformatters/control_code_formatter_test.dart` закрепляют и
пропускание `ESC`, и частичность строгого режима, чтобы будущий safe mode
ломал их намеренно. Релиз 0.7.0 этой находкой не блокируется.

**Вердикт-2 (2026-08-22): исправлено, находка закрыта по существу.** Safe
mode сделан и включён по умолчанию: управляющая последовательность
показывается (`[CSI 2 ED]forged`), а не отправляется, и `ESC` в результате
не остаётся. Покрыты сообщение, текст ошибки, строковые значения, ключи
`Map`, имена свойств, имена enum и текст `LoggableView` — в консоли и в
`FileLogStorage`. Raw ANSI стал явным opt-in
(`LoggableConfig(escapeAnsiCodes: false)`), который политика в
`Loggable.forceConfig` отклоняет, — ровно то распределение, которое
рекомендовало ревью.

Блокер снят цепочкой конфигов (`2026-08-22[6]`), а само двойное применение
value-форматтера исправлено отдельным коммитом как самостоятельный дефект:
`Prop.toLogString` прогонял форматтер поверх уже отрендеренного текста.
Экранирование выполняет `String.ansiShowEscapeSequences()` из
`ansi_escape_codes` — писать своё не понадобилось.

Характеризующие тесты, которые фиксировали принятое поведение, переписаны
под новый контракт: они и были написаны с расчётом, что safe mode сломает их
намеренно. Отчёт — `docs/records/2026-08-22[8]-ansi-safe-mode-report.md`.
...
**Вердикт (2026-08-22): исправлено.** Взят первый из предложенных путей:
`LoggableData` получил библиотечно-приватный геттер `_ownConfig` (`null` по
умолчанию), переопределённый в `_LoggableBuilder` и `_LoggableMapBuilder`, а
`_rootConfig`/`_rootJsonConfig` спрашивают его через общий
`_containerConfig` наравне с `LoggableMultiData`. Одна точка вместо двух
расходящихся веток.

Воспроизведение подтверждено до правки на всех четырёх контейнерах сразу:
при `collectionMaxCount: 1` и замене корня на `[1, 2, 3]` `LoggableMultiData`
и `LoggableWrapper` печатали `[₌₃ ₀:1, …]` и `{:k: list, :l: 3, :v: [1]}`, а
`builder` и `mapBuilder` — полный `[₌₃ ₀:1, ₁:2, ₂:3]` и `[1, 2, 3]`. После
правки все четыре совпадают.

`LoggableWrapper` в `_containerConfig` намеренно не попал: он разворачивает
себя до обхода и предлагает правилу уже содержимое со своим config, поэтому
работал и раньше. Это закреплено отдельным тестом, чтобы следующая правка не
приняла отсутствие ветки за пропуск. Пять тестов в
`sanitizer_root_test.dart`: два RED до правки, три — характеристика соседних
случаев, включая builder без своего config.
...
**Вердикт (2026-08-22): исправлена актуальная часть; часть про BBCode
устарела до начала работ.** Кэша regex по identity темы больше нет:
переписывание парсера по находке №1 заменило backtracking-regex однопроходным
scanner'ом, который собирает набор тегов из темы на каждый вызов. Поэтому тег,
добавленный в изменяемую `messageStyles` после первой печати, применяется со
следующего рендера и от истории вызовов не зависит; закреплено тестом в
`test/preformatters/bb_code_formatter_test.dart`. Кэш уровней темы
(`LogMainTheme.operator []`) хранит `LogTheme._(this, level)` в `Expando` —
ссылку на тему, а не снимок настроек, так что и он мутацию видит.

Актуальной оставалась вторая половина: `ConsoleLogPrinter.output` изменяем, а
`_printers` кэширует `StackedPrinter` на пару `(isActive, level)` с
захваченным при создании sink'ом. Подмена `output` после первой печати
уходила в старый sink для уже напечатанных уровней и в новый — для ещё не
встречавшихся, то есть применялась частично и по истории вызовов. Исправлено
без сброса кэша: принтер уровня пишет через метод `_write`, читающий поле при
каждом вызове, поэтому состояние принтера сохраняется, а замена действует со
следующей строки для всех уровней сразу. Два теста, из них один RED до
правки; второй (новый уровень после подмены) закрепляет половину, которая
работала и раньше.
...
**Вердикт (2026-08-22): исправлено.** `Logger.zonedTags()` возвращает
`UnmodifiableSetView` — симметрично `zonedTraceIds()`. Попытка `add` даёт
`UnsupportedError`, и теги последующих логов зоны остаются прежними. Заодно
характеризован соседний инвариант, который уже работал: `trace()` копирует
переданное множество (`{...zonedTags(), ...tags}`), поэтому вызывающий не
сохраняет пишущую ручку на состояние зоны и позднее добавление в свой `Set`
никуда не просачивается. Три теста, два из них RED до правки. Ломающее для
кода, который мутировал результат getter'а, — пункт в CHANGELOG.
...
**Вердикт (2026-08-22): исправлено.** Сделано ровно то, что предлагала
находка: единая модель ширины в колонках терминала на все операции —
измерение, добивку, обрезку, перенос и выравнивание tail. Спека —
`2026-08-22[3]-display-width-design.md`.

Разбор снял половину предполагаемой работы: апстрим `ansi_escape_codes`
менять не понадобилось. `Parser.substring(start, maxLength:)` адресует
позиции в code units plain-текста, а `String.ansiRemoveEscapeCodes()` —
публичный API того же пакета и, по его собственной документации, читает тем
же шаблоном. Поэтому достаточно надстройки, переводящей бюджет колонок в
число code units: `display_width.dart` (таблицы UAX #11 плюс `ClusterIndex`)
и `measured_line.dart` (пара «парсер + индекс кластеров»).

Ширину кластера задаёт его первый code point: 2 для East Asian Wide и
Fullwidth, 0 для одиноко стоящей combining mark или zero-width символа, 1 для
остального. Марки, joiner'ы и селекторы после базы рисуются внутри неё и
ничего не добавляют, поэтому отдельная таблица combining нужна только для
вырожденного случая «текст начинается с марки».

Побочный, но важный итог: `fixDanglingSurrogates` удалён. Нарезка по границам
кластеров не может разрезать суррогатную пару, поэтому заплатка стала мёртвым
кодом, а два существовавших теста на повисшие половины теперь проходят по
построению, а не потому, что результат чинят после разреза.

Владелец выбрал вариант A — `characters ^1.3.0`. Констрейнт не косметика:
негативная проба показала, что `^1.4.0` роняет version solving на Flutter
3.27 («Consider downgrading your constraint on characters»), потому что SDK
пинит 1.3.0. Матрица Flutter переснята после изменения `dependencies`: восемь
комбинаций из восьми зелёные, на 3.27 резолвится `characters` 1.3.0, на
3.29 — 1.4.0, на 3.44.4 и stable — 1.4.1.

24 теста: 15 на модель ширины и 9 на раскладку. ASCII-вывод не изменился —
`scripts/screenshots.sh --check` даёт код 0, все 41 кадр байт-в-байт прежние.
...
**Вердикт (2026-08-22): исправлено.** `LogElement` заменён на реально
существующий `LogBlock` в README (два блока, три упоминания) и в
`docs/architecture.md`; `test/export_test.dart` теперь проверяет, что имя
экспортировано и встроенные блоки его реализуют, — обещание README стало
проверяемым. Ссылка на LICENSE переписана на абсолютный URL репозитория:
относительный путь чинит README, но ломает `dart doc`, который принимает его
за doc-путь и генерирует ссылку на `LICENSE/index.html`. `example/README.md`
описывает фактический конвейер кадров (`runFrames`, `--list`,
`scripts/screenshots.sh`) вместо ручного комментирования, а «archiving»
заменено на gzip-экспорт, который пример и делает. Противоречие в CHANGELOG
снято: требование `format` ^4.1.0 больше не заявляется как контракт 0.7.0 —
подъём описан как причина, по которой разблокировался Flutter, а итог
(зависимость ушла из пакета, `format` остался только dev-зависимостью тестов)
изложен один раз и без спора с самим собой. `[a, b, c, d]` в dartdoc
экранирован. Критерий закрытия достигнут: `dart doc --validate-links` — **0
предупреждений и 0 ошибок**.
