...
Лог проходит через два затвора, и оба по умолчанию закрыты: `level` логгера
начинается с `LogLevels.off`, а пока не назначен publisher, логу некуда
идти. Просто созданный `Logger` не печатает ничего и не жалуется:

```dart
Logger('app').i('nothing happens'); // ни вывода, ни предупреждения
```

Минимум, который печатает:
...
То же свойство делает эту раскладку предпочтительной, когда логи читает
ИИ-агент. Один лог — одна строка, поэтому grep или фильтр возвращают записи
целиком, а не обрывки, и понимать совпадение не нужно, достраивая его из
соседних строк. Перенесённое сообщение даёт агенту обратное: попадание в
середину, а сама запись размазана по строкам до и после.
    ...
    rows: [
      LogRow(
        ...
        children: [
          ...
          LogTime.onlyTime(),
          LogPath(
            // Место под путь неймспейса расширяется по мере появления
            // длинных путей и обратно не сужается — вплоть до 20 колонок.
...
**Строка добивается пробелами до `maxLength`.** Всё, что короче ширины
строки, несёт хвостовые пробелы до неё. В консоли это и нужно: именно так
фоновый цвет закрывает строку целиком, и именно так держится
правоприжатый `tail`. Если вывод уходит туда, где текст хранят, а не
рисуют, — в файл или в агрегатор логов, — пробелы уходят вместе с ним. У
`LogRow.singleLine` ширины нет, и он не добивает.

**Ширину выбираете вы, и ширину терминала пакет не читает никогда.** Отчасти
потому, что не может: `dart:io` эта библиотека не несёт, а за `output` вовсе
не обязан стоять терминал. Но главное — лог дописывается, а не
перерисовывается. Строка, напечатанная на 120 столбцов, останется такой в
скроллбэке; сужение окна не переверстает то, что уже там. Принтер, следящий
за окном, лишь положил бы в один лог две сетки, а сетка, меняющаяся на
середине, хуже, чем просто слишком широкая. Это задача терминального
интерфейса, и он её решить может — потому что владеет экраном и
перерисовывает его.

Прочитать ширину один раз на старте — пожалуйста, но с запасным значением на
случай, когда терминала нет: в пайпе `stdout.terminalColumns` бросает.
...
А если следить за размером всё-таки хочется, делайте это сами: соберите новый
принтер и присвойте его. `log.publisher = ConsoleLogPrinter(/* ... */)`
действует со следующего лога, и сквозная нумерация переживает подмену.
...
`dart:developer` во Flutter-приложении, в буфер в тесте, в кольцевой буфер
в памяти для отладочного экрана:
...
**Он синхронный** — `void Function(String)`, никакого future, который можно
дождаться, — и поэтому не годится для всего, что вынуждено ждать: для
сокета, HTTP-вызова, файла. Запись туда отсюда либо заблокирует вызов,
который логировал, либо запустит неотслеживаемую работу, чей порядок и чьи
ошибки никто не увидит. Доставка, которая ждёт, — дело публишера, умеющего
копить, отправлять пакетами и сообщать о потерях: ровно это делает
`FileLogStorage`, а для всего остального шов — `CustomLogPublisher`.
...
Значит, sink на `developer.log` создаёт по записи на *строку*, а такому
приёмнику это нужно редко. Если лог должен приходить одним событием,
поставьте `oneCallPerLog`:
...
Тогда все строки всех `rows` одного лога — включая перенесённые —
склеиваются через `\n` и уходят одним вызовом. Текст тот же: меняется только
количество вызовов. Лог, не напечатавший ни строки, не даёт вызова вовсе.
...
log.d(
  ...
  data: buildDiagnostics, // тир-офф работает не хуже, чем () => …()
  tags: collectTags,
);
```
...
![Обрезка Map](screenshots/data_5.png)
...
![Iterable с эффективной длиной](screenshots/data_6.png)
...
![Настройки форматирования. Enum](screenshots/data_7.png)
...
![Настройки форматирования. Числа](screenshots/data_8.png)
...
![Настройки форматирования. Строки](screenshots/data_9.png)
...
идентификатором сессии, временем старта, версией формата и любыми полями,
переданными в `meta`:
...
`formatVersion` говорит, какую форму файла видит читатель: лог-файл
переживает запуск, который его написал, и диагностический архив вполне может
быть открыт много позже инструментом, собранным против другой версии пакета.
Сравнивать — с `FileLogCodec.formatVersion`; поле стоит в каждом чанке
(поэтому чанк, взятый из середины, опознаёт себя сам) и, как `sessionId` и
`started`, не перекрывается через `meta`. Добавление ключа версию не поднимает:
читателю, игнорирующему незнакомые ключи, оно безразлично.
...
**Какой импорт.** `package:team_logger/team_logger.dart` не тянет `dart:io`
и работает везде, где работает Dart, включая web.
`package:team_logger/team_logger_io.dart` — тот же API плюс
`FileLogStorage`, и ему нужна файловая система: мобильные, десктоп, VM, но
не web.

**Где создавать.** Достаточно `final` верхнего уровня: Dart инициализирует
его при первом обращении, так что до первого лога он ничего не стоит.
Настраивайте до `runApp` — или там, где у вас живут долгоживущие объекты.

**Escape-коды на iOS.** `print` их там калечит — цвета приходят мусором
`\^[[31m…`. Либо убрать цвет через `LogMainTheme.noColors`, либо отправлять
строки в `log()` из `dart:developer`: он их сохраняет, но виден только в
DevTools и в отладочной консоли IDE. Подробности и ссылка на upstream — в
разделе 1, «Куда уходит вывод».

**Релизные сборки.** Ставьте `level` логгера в зависимость от `kDebugMode` —
раздел 3, «Логи в релизной сборке».

**Экран с логами внутри приложения.** `LogStorage` держит в памяти последние
N логов ровно для этого; [flutter_team_logger](https://pub.dev/packages/flutter_team_logger)
строит поверх него просмотрщик.

---

## Известные ограничения

- **`config:` на месте вызова оборачивает только `data`.** Сообщение, текст
  ошибки, `units` и `DateTime` через `LoggableConfig` не управляются — их
  задают приложение и тема.
- **Когда правило редактирования выбрасывает *всё* содержимое объекта
  данных, блок данных убирает только `LoggableMultiData`.** Остальные формы
  оставляют на виду пустую оболочку — `login: User()`, `login: {}`,
  `login: Object()`, `login: {₌₁ }`, — поэтому «отрисовалось пустым» и «всё
  выброшено» выглядят одинаково.
- **Настройки размеров файлов — цели ротации и хранения, а не жёсткие
  потолки**: где сессия может их превысить, сказано в разделе 9.
- **То, что рисует `LoggableView`, безопасным режимом ANSI не
  обезвреживается** — намеренно, см. раздел 11. Значение, переданное *в*
  view, обезвреживается.
