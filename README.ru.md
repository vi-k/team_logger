...
группируют в любой локали, `n` — локалезависимая форма, и никакой внешней
локали `format` не читает: `n` следует C-локали, а `Intl.defaultLocale`
ничего не меняет. `'{:,n}'` там
