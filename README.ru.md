...
**На iOS это не вопрос вкуса.** ANSI-коды там ломают вывод `print`, поэтому
приложению, которое работает на iOS, нужен один из двух выходов: убрать коды
через `LogMainTheme.noColors` либо отправлять строки в `log()` из
`dart:developer`, как выше.
